...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/28 10:18:10 by mayyildi          #+#    #+#             */
/*   Updated: 2022/10/28 10:21:09 by mayyildi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*put_line(char *str)
{
	...
	i = 0;
	clean_line = malloc(sizeof(char) * ft_strlen(str) + 2);
	while (str[i] != '\n')
	{
		clean_line[i] = str[i];
		i++;
...
char	*update_storage(char *str)
{
	...
	if (str[i] == '\n')
		i++;
	ustr = malloc(sizeof(char) * (ft_strlen(str) - i + 1));
	if (!ustr)
		return (NULL);
	while (str[i])
	{
...
/*	We pass the address of storage in gnl() to this	*/
